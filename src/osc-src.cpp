...
static void on_process(void *userdata, struct spa_io_position *position) {
  ...
  char buffer[2000];
  std::array<processing::MidiMessageProcessor::parameter_change_event, 1>
      updates{processing::MidiMessageProcessor::parameter_change_event(
          processing::saturator_level_in, utils::Node(), 1, 1.0,
          processing::MidiMessageProcessor::A, 1,
          processing::MidiMessageProcessor::INPUT)};

  auto actual_size =
      osc_service::OscService::build_message(updates, buffer, 2000);

  spa_pod_builder_control(&builder, sample_offset, SPA_CONTROL_OSC);
  spa_pod_builder_bytes(&builder, buffer, actual_size);
...
int main(int argc, char *argv[]) {
  ...
  data.filter = pw_filter_new_simple(
      pw_main_loop_get_loop(data.loop), "osc-src",
      pw_properties_new(PW_KEY_MEDIA_TYPE, "Osc", PW_KEY_MEDIA_CATEGORY,
                        "Playback", PW_KEY_MEDIA_CLASS, "Osc/Source", NULL),
      &filter_events, &data);
